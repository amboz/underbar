(function() {
  ...
  _.filter = function(collection, test) {
    var filtered = (Array.isArray(collection)) ? [] : {};

    _.each(collection, function(item, index) {
      if (test(item, index)) {
        Array.isArray(collection) ? filtered.push(item) : filtered[index] = item;
      }
    });
  ...
  _.reject = function(collection, test) {
    ...
    // var rejected = if(Array.isArray(collection)) ? [] : {};
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (iterator) {
      _.each(array, function(item, index) {
        if (_.indexOf(unique, item) === -1 && iterator(item)) {
          unique.push(item);
        }
      });
    } else {
  ...
  _.extend = function(obj) {
    ...
    for (var i = 0; i < srcs.length; i++) {
      _.each(srcs[i], function(item, index) {
        obj[index] = item;
      });
    }
